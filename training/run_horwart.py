...
if __name__ == '__main__':
    ...
    train_dir = 'data/'
    train_file = 'train_data_hogwart.pickle'
    dev_file = 'dev_data_hogwart.pickle'
    previous_model_file = './chatbot_output/poly_16_pytorch_model_1.bin'
    ...
    ## special_token_load
    import pandas as pd
    sp = pd.read_csv('./data/hogwart_special_token.csv')
    sp_list = []
    ...
    if not args.eval:
        train_dataset = SelectionDataset(os.path.join(train_dir, train_file),
                                                                      context_transform, response_transform, concat_transform, sample_cnt=None, mode=args.architecture)
        val_dataset = SelectionDataset(os.path.join(train_dir, dev_file),
        ...
        val_dataset = SelectionDataset(os.path.join(train_dir, test_file),
                                                                  context_transform, response_transform, concat_transform, sample_cnt=None, mode=args.architecture)
    ...
    state_save_path = os.path.join(output_dir, '{}_{}_pytorch_model.bin'.format(args.architecture, args.poly_m))
    ...
    if args.use_pretrain and not args.eval:
        # previous_model_file = './chatbot_output/poly_16_pytorch_model_17.bin'
        ...
        model.load_state_dict(torch.load(previous_model_file, map_location="cpu"))
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    #     log_wf.write(str(val_result) + '\n')
                        ...
                        #     val_result['best_eval_loss'] = best_eval_loss
                            # save model
                            # state_save_path = os.path.join(output_dir, '{}_{}_pytorch_model_{}.bin'.format(args.architecture, args.poly_m, epoch))
